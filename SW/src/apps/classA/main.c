 ...
 * Modbus Variable
 */
uint8_t mbData[40*sizeof(int16_t)];
...
void MbMcuIrqNotify( UartNotifyId_t id )
{
	uint8_t data;
...
//	printf("Sending Data....!\r\n");
	mbLen = 0;
	for (int i = 0; i < mstatus.request.length; i++ ){
		m_printchar(mstatus.request.frame[i]);
		printf("\r\n %d %x" ,i,mstatus.request.frame[i]);
	}
	printf("\r\n");
	GpioWrite(&GpsPowerEn, 0);
	DelayMs(1480);

	// Passing Data of master
	mstatus.response.frame = mbData;
	mstatus.response.length = mbLen;
...
void masterdump( )
{
	...
	printf( "Received data: slave: %02x, addr: %02x, count: %02x, type: %02x\r\n",
	...
	switch ( mstatus.data.type )
	{
		...
		case MODBUS_INPUT_REGISTER:
			for ( i = 0; i < mstatus.data.count; i++ )
				printf( " %02x", mstatus.data.regs[i] );
			break;
	...
	for ( i = 0; i < mstatus.request.length; i++ )
		printf( " %02x", mstatus.request.frame[i] );
	printf( "\r\n" );

	printf( "Response:" );
	for ( i = 0; i < mstatus.response.length; i++ )
		printf( " %02x", mstatus.response.frame[i] );
	printf( "\r\n" );
...
static void PrepareTxFrame(uint8_t port) {
	...
	case 2: {
		if (ret == SUCCESS) {
			for ( i = 0; i < mstatus.response.length; i++ ){
				AppData[i] = mstatus.response.frame[i];
				printf(" %02X",AppData[i]);
			}
			AppDataSize = mstatus.response.length + 1;
			printf("\r\nAppData Size = %s\r\n\r\n",AppDataSize);
		}
		else {
			AppDataSize = 0;
			MODBUS_GETFAIL = SUCCESS;
		}
...
static void McpsConfirm(McpsConfirm_t *mcpsConfirm) {
	...
	if (mcpsConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK) {
		...
		case MCPS_UNCONFIRMED: {
			printf("\r\n\t\ttMcpsConfirm - MCPS_UNCONFIRMED\r\n");
			// Check Datarate
			// Check TxPower
			break;
		}
		case MCPS_CONFIRMED: {
			printf("\r\n\t\ttMcpsConfirm - MCPS_CONFIRMED\r\n");
			// Check Datarate
			// Check TxPower
			// Check AckReceived
			// Check NbTrials
...
static void McpsIndication(McpsIndication_t *mcpsIndication) {
	...
	case MCPS_UNCONFIRMED: {
		printf("\r\n\t\tMcpsIndication - MCPS_UNCONFIRMED\r\n");
		break;
	}
	case MCPS_CONFIRMED: {
	...
	if (mcpsIndication->RxData == true) {
		...
		printf("\r\nRecived Something on Port : %d\r\n",mcpsIndication->Port);
		case 1: // The application LED can be controlled on port 1 or 2
		case 2:
			if (mcpsIndication->BufferSize == 1) {

				AppLedStateOn = mcpsIndication->Buffer[0] & 0x01;
				if(((AppLedStateOn & 0x01) != 0)){
...
//				}
			...
			if (ComplianceTest.Running == true) {
				printf("\r\nDeviceState - DEVICE_STATE_SEND - ComplianceTest.Running\r\n");
...
//	MX_IWDG_Init();
	...
	GpsUart.IrqNotify = MbMcuIrqNotify;
	UartMcuInit(&GpsUart, UART_3, GPS_UART_TX, GPS_UART_RX);
	UartMcuConfig(&GpsUart, RX_TX, 9600,
								UART_8_BIT,
								UART_1_STOP_BIT,
								NO_PARITY,
								NO_FLOW_CTRL);
...
//	GpioInit( &GpsPowerEn, GPS_POWER_ON_PIN, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
	GpioInit( &GpsPowerEn, REDE_PIN, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
	GpioWrite(&GpsPowerEn,0);
	...
	modbusBuildRequest03( &mstatus, 5, 01, 32 );


	DeviceState = DEVICE_STATE_INIT;
	printf("punit soft version: 1.1.3\r\n");
